<?php 
	...
	class Manage_group extends BaseApiController
	{
		...
		public function add(){
			...
			if(!$this->manage_group_model->create($data)){
		...
		public function edit(){
			...
			$data['name'] = $name;
			$this->load->model('manage_group_model');
			if(!$this->manage_group_model->update($id,$data)){
		...
		public function del(){
			...
			$this->load->model('manage_group_model');
	       	$userGroup = $this->manage_group_model->getByIds($id);
	       	// var_dump($userGroup);exit;
	       	$data = array();
	       	$data['is_delete'] = EnumRes::DELETE_YES;
	       	if(isArray($userGroup)){
	           foreach($userGroup as $value){
	              if(!$this->manage_group_model->update($value['id'],$data)){
	                  $this->outputFail('操作失败');
	               }
	           }  
	       	}
		...
		public function detail(){
			...
			$this->load->model('manage_group_model');
			$userGroup = $this->manage_group_model->load($id);
			$data = array();
			if($userGroup){
		...
		public function index(){
        	...
        	$this->load->model('manage_group_model');
        	$total = $this->manage_group_model->count($options);
        	if($total){
        		$list = $this->manage_group_model->find($options,$pageSize, ($page - 1) * $pageSize);
        		$tmp = array();
